...
def test_readme_has_correct_badge():
    ...
    for readme_file in get_all_readme_files(ROOT_PATH):
        try:
            # dir_name is the relative path of the README's parent directory from ROOT_PATH
            # e.g., "ai-chat-assistant" or "devops/purrr"
            dir_name = readme_file.parent.relative_to(ROOT_PATH).as_posix()

            if not dir_name or dir_name == ".":
                errors.append(
                    f"README file: {readme_file.relative_to(ROOT_PATH)}\n"
                    f"  Could not determine a valid directory name for the badge link."
                )
                continue
            ...
            if expected_badge_markdown not in content:
                errors.append(
                    f"README file: {readme_file.relative_to(ROOT_PATH)}\n"
                    f"  Missing or incorrect badge.\n"
                    f"  Expected: {expected_badge_markdown}\n"
                )
        except FileNotFoundError:
            errors.append(f"README file not found (unexpected): {readme_file.relative_to(ROOT_PATH)}")
        except UnicodeDecodeError:
            errors.append(f"README file: {readme_file.relative_to(ROOT_PATH)}\n  Could not be read as UTF-8.")
        except Exception as e:
            errors.append(
                f"Error processing file {readme_file.relative_to(ROOT_PATH)}: {type(e).__name__} - {e}"
            )
