...
def on_slack_whatsfree_slash_command(event):
    print(f"on_slack_whatsfree_slash_command()")
    ...
    for room in rooms:
        ...
        try:
            ...
            if not events:
                slack.chat_postMessage(channel=channel_id, text=f"{room} is free for the next 30 minutes")
                available = True
    ...
    if not available:
            slack.chat_postMessage(channel=channel_id, text="No free rooms found for the next 30 minutes")
